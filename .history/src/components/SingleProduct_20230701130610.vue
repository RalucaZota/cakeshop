<template>
    <article class="product-article" v-for="{id, image, name, price, quantity, description, ingredients} in  singleProduct" :key="id">
        <div class="product-wrapper-div-img">
            <img :src="image" class="single-product-img" ref="canvas"   @mouseenter="handleMouseEnter"
            @mouseleave="handleMouseLeave" :style="{ opacity: isHovered ? '0' : '1' }" @mousemove="updateCanvas"/>
        </div>
